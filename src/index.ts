...
const app = new Elysia()
  .use(cors({
    origin: config.elysiaJs.ALLOWED_ORIGINS || '*',
...
app.onRequest(({ set }) => {
  ...
  set.headers['Content-Security-Policy'] = "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; img-src 'self' data: https:; font-src 'self' data: https:;";
...
// Mount the controller directly (no group)
urlController(app);
...
try {
  ...
  app.listen(port, () => {
    console.log(`🦊 URL Shortener is running at ${config.elysiaJs.BASE_URL}:${port}`);
    console.log(`📚 Swagger documentation available at ${config.elysiaJs.BASE_URL}:${port}/swagger`);
  });
} catch (error) {
